...
v = Operators.getcappedyukawa(lat; format=:dense, cellrange=3, spin=true, a=4.0, U=3.6) # interaction potential
...
for (i_, filling) in enumerate(fillings)
    ...
    ρ_sol, ϵ_GS, HMF, converged, error = Meanfield.solvehartreefock( # run the calculation
        H, v, ρ_init, filling; klin=50, iterations=900, tol=1e-5,# p_norm=Inf,
