...
FileIO.save("output_mf_sweep/interaction_params.jld2", 
    Dict("a"=>a, "U"=>U)
)

v = Operators.getcappedyukawa(lat; format=:dense, cellrange=3, spin=true, a=a, U=V) # interaction potential
...
for (i_, filling) in enumerate(fillings)
    ...
    if isfile("output_mf_sweep/meanfield$i_.jld2")
        println("  (Mean-field file exists. Will use it as initial guess!)")
        ρ_sol = FileIO.load("output_mf_sweep/meanfield$i_.jld2", "ρ_sol")
    end

    Operators.setfilling!(H, filling; nk=100^2)

    ρ_sol, ϵ_GS, HMF, converged, error = Meanfield.solvehartreefock( # run the calculation
        H, v, ρ_init, filling; klin=50, iterations=500, tol=1e-5,# p_norm=Inf,
        T=0.002, β=0.45,  show_trace=true, clear_trace=true, multimode=:distributed
    )
