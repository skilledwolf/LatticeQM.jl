...
# direction (e.g. if eigen_sparse_arpack.jl is re-enabled by accident, or if a
# future KrylovKit version changes Lanczos behaviour).
@testset "Eigen.eigen_sparse: small dense reference" begin
    ...
    N = 60
    rng_seed = 0x123456789abcdef
    A = SparseMatrixCSC{ComplexF64}(sparse(Symmetric(rand(N, N) .- 0.5)))
    A_dense = Matrix(A)
