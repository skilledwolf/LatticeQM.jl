...
v = Meanfield.gethubbard(lat; mode=:σx, a=0.5, U=3.0) # interaction potential
...
ρ_sol, ϵ_GS, HMF, converged, error = Meanfield.solvehartreefock( # run the calculation
    hops, v, ρ_init, filling; klin=6, iterations=400, tol=1e-6,# p_norm=Inf,
    T=0.002, β=0.93, show_trace=true, clear_trace=true, verbose=false, multimode=:parallel
)
...
Operators.setfilling!(hops, filling; nk=9)
...
hmf = HMF.h
Operators.setfilling!(hmf, filling; nk=9)
bands_mf = getbands(hmf, ks, sz; format=:sparse, num_bands=36)
