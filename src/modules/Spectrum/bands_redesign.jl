...
################################################################################
# Special type for (lazy) discretized Hamiltonian
################################################################################

abstract type AbstractDiscretizedHamiltonian{T1,T2} end

struct DiscretizedHamiltonian{T1,T2} <: AbstractDiscretizedHamiltonian{T1,T2}
    h::T1
    ks::T2
end

# (s::AbstractDiscretizedHamiltonian)(k) = s.h(k)
Base.getindex(dh::AbstractDiscretizedHamiltonian, i::Int) = dh.h(dh.ks[:, i])

# Iterator and length for both types
# it might make sense to introduce numkpoints instead of size(...,2) here
function Base.iterate(dh::AbstractDiscretizedHamiltonian, state=1) 
    state > size(dh.ks,2) ? nothing : (dh[state], state + 1)
end

Base.length(dh::AbstractDiscretizedHamiltonian) = size(dh.ks,2)

dim(dh::AbstractDiscretizedHamiltonian) = dim(dh.h, dh.ks) # default fallback definition

bandmatrix_size(dh::DiscretizedHamiltonian; kwargs...) = bandmatrix_size(dh.h, dh.ks; kwargs...)


import SharedArrays

abstract type AbstractBandData{T1} end

struct BandData{T1} <: AbstractBandData{T1}
    bands::Array{Float64,2}
    obs::Array{Float64,3}
    kdata::T1

    BandData(bands::AbstractMatrix, obs::AbstractArray, kdata::T) where {T} = new{T}(SharedArrays.sdata(bands), SharedArrays.sdata(obs), path)
end

struct SharedBandData{T1} <: AbstractBandData{T1}
    bands::SharedArray{Float64,2}
    obs::SharedArray{Float64,3}
    kdata::T1

    BandData(bands::AbstractMatrix, obs::AbstractArray, kdata::T) where {T} = new{T}(bands, obs, path)
end

# we could add some type aliases for the specific case where kdata is a path object 
