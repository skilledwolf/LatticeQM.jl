...
function BerryF(StatesGrid::T) where T<:AbstractArray{<:Complex, 4}
...
function BerryCurvature(wavefunctions::Function, NX::Int, NY::Int=0, bandindex=1)
    if NY < 1
        NY = NX
    end
...
"""
    ...
    @sync @distributed for j_=2:N+1
        ...
        δkU = (rot(π/2)*δkR + rot(-π/2)*δkL)/2
        δkD = (rot(-π/2)*δkR + rot(π/2)*δkL)/2
...
function get_berry!(bands::BandData, h, ks)
    obs = Array(BerryCurvature(Us(h), ks.points))
...
#     p = plot()
#
#     K(k::T) where T<:AbstractArray{Float64,N} where N = Structure.get_B(lat) * k
#
