...
solveselfconsistent(hf, ρ_init::AnyHops, filling::Number, args...; kwargs...) = solveselfconsistent(ρ_init, hf..., filling, args...; kwargs...)
...
function solveselfconsistent!(ρ0::AnyHops, ρ1::AnyHops, ℋ_op::Function, ℋ_scalar::Function, filling::Float64, ks::AbstractMatrix{Float64};
    ...
    """
    ρ0 = Hops(δL=>SharedArray(complex(m)) for (δL, m)=ρ0)
    ρ1 = Hops(δL=>SharedArray(complex(m)) for (δL, m)=ρ1)
    ...
    function update!(ρ1, ρ0)
        ...
        ϵ0 = densitymatrix!(ρ1, H.h, ks, H.μ; T=T, format=:dense) # get new meanfield and return the groundstate energy (density matrix was written to ρ1)
