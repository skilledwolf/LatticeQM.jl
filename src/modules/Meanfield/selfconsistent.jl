...
function solveselfconsistent!(ρ0::AnyHops, ρ1::AnyHops, ℋ_op::Function, ℋ_scalar::Function, filling::Float64, ks::AbstractMatrix{Float64};
    ...
    ρ0 = Hops(δL=>ifelse(parallel, SharedMatrix(complex(m)), Matrix(complex(m))) for (δL, m)=ρ0) # convert to dense
    ρ1 = Hops(δL=>ifelse(parallel, SharedMatrix(complex(m)), Matrix(complex(m))) for (δL, m)=ρ1) # convert to dense
